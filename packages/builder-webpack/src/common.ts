...
export const customWebpackConfigTransformFactory: (
    ...
    const variantPath = `${getSystemPath(normalize(context.workspaceRoot))}/projects/${context.target.project}/src/variants/${variantFileName}.ts`
...
export const getTransforms = (options: ElementVariantsWebpackSchema, context: BuilderContext): {
...
export function fromTargetOptions<T>(context: BuilderContext, browserTarget: string): Observable<T> {
...
export function mergeOptions<T>(options: T, context:  BuilderContext): Observable<T> {
    ...
    */
    ...
    const optionsRemote$ = optionsSourceOverriddenByConsole$.pipe(
        switchMap((options) => {
            return 'browserTarget' in options ? fromTargetOptions<T>(context, options['browserTarget'])  : of({} as ElementVariantsWebpackSchema);
        }));
    return zip(optionsRemote$, optionsSourceOverriddenByConsole$)
        .pipe(
