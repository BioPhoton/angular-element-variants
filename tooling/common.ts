...
export function copyPackageDefaults(
  files: string[],
  source: string,
  destination: string
): Observable<void> {
  const observables: Observable<void>[] = files.reduce(
    (obs, file) =>
      obs.concat([from(fs.copyFile(path.join(source, file), path.join(destination, file)))]),
    []
  );
...
export function syncWithNodeModules(source: string, destination: string): Observable<void> {
  return concat(from(fs.copy(source, destination)));
}
